...
class FCSN_1D_unsup_self(nn.Module):
    def __init__(self, n_class=2):
        super(FCSN_1D_unsup_self, self).__init__()
        ...
        self.deconv1 = nn.ConvTranspose1d(n_class, n_class, 4, padding=1, stride=2, bias=False)
        self.deconv2 = nn.ConvTranspose1d(n_class, n_class, 16, stride=16, bias=False)
        self.deconv_conv7 = nn.ConvTranspose1d(4096, 4096, 32, stride=32, bias=False)

        self.conv_reconstuct1 = nn.Conv1d(4096, 1024, 1)
        self.bn_reconstruct1 = nn.BatchNorm1d(1024)
        self.relu_reconstuct1 = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        h_reconstruct = self.relu_reconstuct1(self.bn_reconstruct1(self.conv_reconstuct1(h_conv7_deconv))) # [5,1024,320]

        # merge with input features
        #h_merge = h_reconstruct + x # [5,1024,320]
        #h_merge_reconstruct = self.relu_reconstuct2(self.bn_reconstruct2(self.conv_reconstuct2(h_merge))) # [5,1024,320]

        return h_reconstruct, mask, h # [5,1024,320], [5,1,320], [5,2,320]

class FCSN_2D_sup(nn.Module):
    def __init__(self, n_class=2):
        super(FCSN_2D_sup, self).__init__()
...
class FCSN_2D_unsup(nn.Module):
    def __init__(self, n_class=2):
        super(FCSN_2D_unsup, self).__init__()
    ...
    def forward(self, x):
        ...
        h_reconstruct = self.relu_reconstuct1(self.bn_reconstruct1(self.conv_reconstuct1(h_mask))) # [5,1024,1,320]
        x_select = in_x*mask

        # merge with input features
        h_merge = h_reconstruct + x_select # [5,1024,1,320]
        h_merge_reconstruct = self.relu_reconstuct2(self.bn_reconstruct2(self.conv_reconstuct2(h_merge))) # [5,1024,1,320]


        return h_merge_reconstruct,mask,h   # [5,1024,1,320],[5,1,1,320],[5,2,1,320]
...
if __name__ == '__main__':
    ...
    # model = FCSN_1D_sup(n_class=2)
    # model.to(device)
    # data = torch.randn(5,1024,320).to(device)
    # print(model(data).shape) # [5,2,320]

    #model = FCSN_1D_unsup_self(n_class=2)
    #model.to(device)
    #data = torch.randn(5,1024,320).to(device)
    #print(model(data)[0].shape) # [5,2,320]


    model = FCSN_2D_unsup(n_class=2)
    model.to(device)
    data = torch.randn(5, 1024, 1, 320, requires_grad=True).to(device)
