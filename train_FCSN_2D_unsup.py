...
for i in range(len(train_loader_list)):
    ...
    for epoch in range(EPOCHS):
        for batch_i, (feature,label,_) in enumerate(train_loader_list[i]):
            ...
            outputs_reconstruct,mask,_ = model(feature) # output shape [5,1024,1,320],[5,1,1,320]

            # 2D 1D conversion
            outputs_reconstruct = outputs_reconstruct.view(5,1024,320)
            mask = mask.view(5,1,320)
            feature = feature.view(5,1024,320)
